...
namespace DocumentRefLoader.Yaml
{
    class ForcedNodeTypeResolver : INodeTypeResolver
    {
        ...
        private readonly IEnumerable<(Func<string, bool> predicate, Type type)> _regexPatternTypes;

        public ForcedNodeTypeResolver(INodeTypeResolver inner, IDictionary<Func<string, bool>, Type> overrideRegexPatternTypes = null)
        {
            _inner = inner;
            _regexPatternTypes = (
                overrideRegexPatternTypes ?? new Dictionary<Func<string, bool>, Type>()
                {
        ...
        public bool Resolve(NodeEvent nodeEvent, ref Type currentType)
        {
            switch (nodeEvent)
            {
                case Scalar scalar:
                    ...
                    foreach (var pat in _regexPatternTypes)
                    {
